...
SQLITE_FILE_PATH = "/Library/Application Support/com.apple.idleassetsd/Aerial.sqlite"

def get_aerials():
    """
    Get the list of aerials from JSON or SQLite depending on what exists.
    """
    aerials_list = []
    
    if os.path.exists(JSON_FILE_PATH):
        print(f"Loading aerials from JSON: {JSON_FILE_PATH}")
        with open(JSON_FILE_PATH) as f:
            d = json.load(f)
            for aerial in d["assets"]:
                aerials_list.append(aerial)
    elif os.path.exists(SQLITE_FILE_PATH):
        print(f"Loading aerials from SQLite: {SQLITE_FILE_PATH}")
        con = sqlite3.connect(SQLITE_FILE_PATH)
        cur = con.cursor()
        try:
            cur.execute("""
                SELECT ZID, ZACCESSIBILITYLABEL, ZLOCALIZEDNAMEKEY, ZURL4KSDR240FPS
                FROM ZASSET
                WHERE ZURL4KSDR240FPS IS NOT NULL
            """)
            for row in cur.fetchall():
                aerials_list.append({
                    "id": str(row[0]),
                    "accessibilityLabel": row[1],
                    "localizedNameKey": row[2],
                    "url-4K-SDR-240FPS": row[3]
                })
        finally:
            con.close()
    else:
        raise FileNotFoundError("No aerial metadata found (entries.json or Aerial.sqlite missing).")
